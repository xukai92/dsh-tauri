...
|---|---|---|---|---|
| dsh | 发布集：非 experimental 的 `packages/*/*` + `apps/cli` + `apps/web`；私有实验性包仅加入共享版本 bump，私有 Tauri manifest 保持在 npm 发布之外 | 发布集、私有 dsh 包与 workspace 根共用一个 `0.0.x` | `dsh-v<版本>` | `release.yml`（pack）/ `release-publish.yml`（发布） |
