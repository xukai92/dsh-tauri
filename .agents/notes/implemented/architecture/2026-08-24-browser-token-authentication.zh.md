...
随附 CLI 支持显式 `--host 0.0.0.0`，并使用派生的 LAN authority 与可选的 `--trusted-host` authority。认证不提供 TLS、转发 header 解释或代理配置；[经认证的远程 Web 设置](2026-09-15-authenticated-remote-web-settings.zh.md)持有该部署姿态。
