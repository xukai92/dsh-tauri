...
Each connection creates a content window and destroys the previous one instead of navigating the existing webview. WebKit withholds a `SameSite=Strict` cookie set during a redirect when the navigation began on another site, so navigating from the loaded local GUI to a remote Host would exchange the token and then receive the unauthenticated response. A new window performs the same first navigation a browser performs when the URL is opened directly. Content-window labels rotate under the `main*` capability glob.
