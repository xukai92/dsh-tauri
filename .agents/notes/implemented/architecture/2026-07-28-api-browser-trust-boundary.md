...
- A non-loopback deployment must trust its serving authorities or requests are refused, then satisfy browser authentication like every loopback request. The shipped CLI derives LAN IP authorities for `--host 0.0.0.0`; `--trusted-host` adds named authorities to the Host/Origin fence and grants no identity. [Authenticated remote Web settings](2026-09-15-authenticated-remote-web-settings.md) owns the shipped deployment posture.
