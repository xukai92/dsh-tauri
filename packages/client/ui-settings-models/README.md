...
The notice step owns its exact copy in `src/client/locales.ts` and its acknowledgement version in `src/onboarding-copy.ts`; it compares and writes `ui-onboarding.welcomeNoticeVersion` through the authenticated settings API for loopback and trusted remote authorities, and only an explicit Continue records the current version. The DeepSeek step targets `deepseek-official` in `llm-deepseek` and renders the existing `ProviderEditor` in credential-only mode inside the shared onboarding modal; `credentials.set` stays the only secret write, and no provider settings are changed.
