 ...
 * the transport: production browsers follow the durable Host section, while a
 * memory-mode scope keeps the acknowledgement process-local here.
 */
...
/** Coordinates durable Host acknowledgement or a process-local memory fallback. */
export class WelcomeNoticeStore {
  ...
  /**
   * Persist this copy version, or advance only this process for a memory-mode
