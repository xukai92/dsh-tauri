...
A row's **package name** resolves from the harness root runtime, not from the active profile or preset directory. Nested profile contexts carry the profile file's `baseUrl`, while the root retains the installed runtime base that can reach packaged plugins. The mount sends bare specifiers to that root base.
