...
The production Web runner needs built package and frontend artifacts (`pnpm run build`). It serves `http://127.0.0.1:3080` by default and, for a local launch, opens that canonical host URL only after the complete Loader tree settles. A non-empty inherited `SSH_CONNECTION` or `SSH_TTY` suppresses the browser handoff because the SSH client or editor owns the local forwarded address; the host URL is still printed. `--host 0.0.0.0` binds all interfaces, prints one discovered LAN URL when available, and adds the discovered LAN IPv4 literals to the `/api` browser-trust fence; `--trusted-host` adds named serving authorities. Every browser still exchanges the launch token and presents its authority-bound cookie, and the server provides neither TLS nor reverse-proxy forwarding-header policy. Immediately before a local handoff it prints `dsh web: opening the default browser; pass --no-open to disable`; if the operating-system handoff fails, a diagnostic on stderr states the reason, leaves the server running, and names the URL for manual use.
