...
1. `run()` 以 `--profile web --port 0 --no-open` 启动打包的 `dsh-web` sidecar：该单文件 exe 由 `scripts/build-tauri-sidecar.ts` 构建（在 web profile 闭包上使用 `@yao-pkg/pkg --sea`），并通过 Tauri `externalBin` 嵌入；`--port 0` 让操作系统选择空闲端口，因此两个 shell 永远不会冲突。当相邻的打包 sidecar 不存在时，`DSH_BIN` 会先于 `PATH` fallback 提供开发二进制文件。
...
第一步把 `dsh-web` sidecar、node-pty 的 `dsh-web-spawn-helper` 和 sharp libvips 库构建到 `apps/tauri/binaries/`；然后 `tauri build` 将三者打包。输出为 `src-tauri/target/release/bundle/macos/DeepSeek Harness.app` 和一个 `.dmg`。注意：构建 `.app`/`.dmg` 需要 macOS（bundle、代码签名和 `icon.icns` 处理都是 macOS 专用步骤）；在安装 WebKitGTK 4.1 开发包后，Rust 本身的 `cargo check`/`cargo build` 可在 Linux 上工作。
...
- **原生附属文件。** sidecar 带有 node-pty 的可执行文件 `dsh-web-spawn-helper` 和 `sharp` 的 libvips 库。macOS 工作流会检查它们在最终 `.app` 中的位置，然后覆盖已签名 sidecar 的前端、RPC、图像、持久终端、设置和关闭路径。它也会启动应用并请求正常的 macOS Quit。
